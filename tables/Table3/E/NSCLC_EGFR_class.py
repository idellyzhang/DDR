...
pattern = 'NSCLC_EGFR_feature.csv'
data_r = pd.read_csv(pattern)

data = biomarker_selection(data_r,18)
...
WT_acc = []
for x in range(1000):
	    X_MU_train, X_MU_test, y_MU_train, y_MU_test = train_test_split(X_MU, y_MU,test_size=0.2)
	    X_WT_train, X_WT_test, y_WT_train, y_WT_test = train_test_split(X_WT, y_WT,test_size=0.2)
...
print("egfr mut: ", mean(MU_acc))
print("egfr wt: ", mean(WT_acc))
